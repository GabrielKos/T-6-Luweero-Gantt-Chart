    ...
    <header className="bg-white shrink-0 border-b border-slate-200 z-30 shadow-xs">
      ...
      <div className="px-4 sm:px-8 py-3.5 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div className="flex items-center gap-4 sm:gap-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="px-2 py-0.5 bg-blue-50 text-[10px] rounded-md text-blue-700 font-bold uppercase tracking-wider border border-blue-200 flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-blue-500" /> Active Model V8.3
              </span>
            </div>
        ...
        <div className="flex flex-wrap gap-2.5 items-center w-full lg:w-auto">
          ...
          <div className="bg-emerald-50/80 border border-emerald-200/80 rounded-lg px-3 py-1.5 flex items-center gap-2.5 text-emerald-900">
            <div className={`w-2.5 h-2.5 rounded-full ${syncStatus === 'synced' ? 'bg-emerald-500' : 'bg-amber-500 animate-ping'}`}></div>
            <div>
              <div className="text-[9px] uppercase tracking-wider text-emerald-700 font-bold leading-none mb-0.5">Cloud Database</div>
              <div className="text-xs font-semibold flex items-center gap-1 leading-none text-emerald-800">
                <Database className="w-3.5 h-3.5" /> 
                {syncStatus === 'synced' ? 'Synchronized' : 'Connecting...'}
              </div>
            </div>
          </div>
